      ...
      <NCard>
        <div class="h-full flex flex-col items-center gap-5">
          ...
          <main class="w-full grow">
            {current.value === 1 && <WorkFlowBasicInfo />}
            {current.value === 2 && <WorkflowDesigner />}
          </main>
        </div>
      </NCard>
    );
...
const WorkFlowBasicInfo = defineComponent<{}>(() => {
  return () => (
    <div class="bg-fill2 p-10 border border-rim2 rounded">
      <SystemForm>
...
const WorkflowDesigner = defineComponent<{}>(() => {
  return () => <AkirDesigner />;
});
